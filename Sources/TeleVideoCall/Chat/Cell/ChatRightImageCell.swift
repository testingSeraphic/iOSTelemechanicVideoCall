...
class ChatRightImageCell: UITableViewCell {
    ...
    func setup(with data: ChatMessage) {
        ...
        configureMessageReadStatus(with: data)
...
//                messageImageView.sd_setShowActivityIndicatorView(true)
    ...
    func configureImageView(from urlString: String) {
        messageImageView.setImage(from: urlString)
        //        messageImageView.sd_setImage(with: URL(string: urlString), placeholderImage: nil)
        //        messageImageView.sd_setShowActivityIndicatorView(true)
    }
